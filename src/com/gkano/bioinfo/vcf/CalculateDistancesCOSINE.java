...
class CalculateDistancesTask extends RecursiveTask<double[][]> {
	...
	private final VariantManager vm;
	private final boolean ignoreHets, onlyHets, ignoreMissing;
...
class CalculateDistancesChildTask extends RecursiveAction {
	...
	protected void compute() {
		try {
			...
			int step = GeneralTools.getAdaptiveSampleStep(count);
			if(count % step == 0 && CalculateDistancesCOSINE.verbose) {
