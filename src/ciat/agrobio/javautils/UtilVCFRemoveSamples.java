/*
 *
 * BioInfoJava-Utils ciat.agrobio.javautils.UtilVCFRemoveSamples
 *
...
@Parameters(commandDescription = "VCFRemoveSamples")
public class UtilVCFRemoveSamples {

	private static UtilVCFRemoveSamples instance = new UtilVCFRemoveSamples();

	private UtilVCFRemoveSamples() {
	}

	public static UtilVCFRemoveSamples getInstance() {
		return instance;
	}

	public static String getUtilName() {
		return "VCFRemoveSamples";
	}
	...
	@Parameter(names={"--sampleList", "-s"}, description = "Sample_List_File", required = true)
	...
	public void go() {
		try {
			...
			//Read sampleList to remove
			List<String> sampleNamesRemove = new ArrayList<String>();
			Scanner scanner = new Scanner(new File(sampleListFileName));
			while(scanner.hasNextLine()) {
				sampleNamesRemove.add(scanner.nextLine().trim().split("[\\s,=]+")[0].trim());
			}
			scanner.close();
			
			//Write final VCF keep only the samples in sampleNameKeep
			...
			for(String sampleName : sampleNames){
				if(!sampleNamesRemove.contains(sampleName))
